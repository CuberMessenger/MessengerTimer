...
using static CSMin2Phase.CubieCube;
...
namespace CSMin2Phase {
    ...
    */
    public class Search {
        ...
        */
        public static int PARTIAL_INIT_LEVEL = 1;

        //Options for research purpose.
        static int PRE_MOVE_LEVEL = 2; //0: disable pre-scramble, 1: 1-move pre-scramble, 2: 2-move pre-scramble
        static bool TRY_INVERSE = true;
        static bool TRY_THREE_AXES = true;
        public static bool USE_CONJ_PRUN = USE_TWIST_FLIP_PRUN;

        static int MAX_DEPTH2 = 13;

        static int PRE_IDX_MAX = PRE_MOVE_LEVEL == 2 ? 113 :
                                       PRE_MOVE_LEVEL == 1 ? 9 : 1;
        static int PRE_IDX_VALID_MAX = PRE_IDX_MAX / 2 + 1;
        static int PRE_IDX_MIN = 9;
        static int PRE_IDX_VALID_MIN = PRE_IDX_MIN / 2 + 1;
        ...
        public string GetSolution(string facelets, int maxDepth, long probeMax, long probeMin, int verbose) {
            //CM
            CubeError check = verify(facelets);
            if (check != CubeError.Solvable)
                return "Error " + check.ToString();
            //CM
            this.sol = maxDepth + 1;
            this.probe = 0;
        ...
        public static bool isInited() => inited;
        ...
        public CubeError verify(string facelets) {
            int count = 0x000000;
        ...
        void initConjPreIdxRange(int urfIdx, int preIdxAdjStart, int preIdxAdjEnd, bool initPhase2) {
            ...
            for (int j = preIdxAdjStart; j < preIdxAdjEnd; j++) {
                pc.copy(ccc);
                foreach (int m in preMoveList[j << 1]) {
                    CornMult(moveCube[m], pc, pc2);
                    EdgeMult(moveCube[m], pc, pc2);
                    pc.copy(pc2);
                }
        ...
        private string search() {
            if (!isRec)
                for (int i = 0; i <= 20; i++)
                    searchTasks[i] = (char)0; // all tasks not finished
            ...
            for (length1 = isRec ? length1 : 0; length1 < sol; length1++) {
                ...
                for (urfIdx = isRec ? urfIdx : 0; urfIdx < 6; urfIdx++) {
                    ...
                    for (preIdx = isRec ? preIdx : preIdxStart; preIdx < preIdxEnd; preIdx += 2) {
                        ...
                        depth1 = length1 - preMoveList[preIdx].Length;

                        if (node0[urfIdx, preIdxValid].prun <= depth1 && phase1(node0[urfIdx, preIdxValid], (int)selfSym & preMoveSym[preIdx], depth1, -1) == 0)
                            return solution == null ? "Error 8" : solution;
                    }

                    searchTasks[length1] = (char)(searchTasks[length1] | (preIdx >= preIdxMin ? 1 : 0) << (urfIdx << 1));
                    searchTasks[length1] = (char)(searchTasks[length1] | (preIdx >= preIdxMax ? 3 : 0) << (urfIdx << 1));
        ...
        private int phase1(CoordCube node, int ssym, int maxl, int lm) {
            ...
            for (int axis = 0; axis < 18; axis += 3) {
                ...
                for (int power = 0; power < 3; power++) {
                    ...
                    move[depth1 - maxl] = m;
                    int ret = phase1(nodeUD[maxl], ssym & (int)moveCubeSym[m], maxl - 1, axis);
                    if (ret == 0)
                        return 0;
                    else if (ret == 2)
                        break;
        ...
        private int phase1opt(CoordCube ud, CoordCube rl, CoordCube fb, long ssym, int maxl, int lm) {
            ...
            for (int axis = 0; axis < 18; axis += 3) {
                ...
                for (int power = 0; power < 3; power++) {
                    ...
                    move[length1 - maxl] = m;
                    int ret = phase1opt(nodeUD[maxl], nodeRL[maxl], nodeFB[maxl], ssym & moveCubeSym[m], maxl - 1, axis);
                    if (ret == 0)
                        return 0;
                }
            }
        ...
        void initPhase2State() {
            ...
            if (preIdx % 2 == 0) {
                ...
                for (int i = 0; i < depth1; i++) {
                    int m = move[i];
                    p2corn = CoordCube.CPermMove[p2corn, Util.std2ud[SymMove[p2csym, m]]];
                ...
                int m = Util.std2ud[preMoveList[preIdx][preMoveList[preIdx].Length - 1] / 3 * 3 + 1];
        ...
        private int initPhase2() {
            ...
            for (depth2 = maxDep2 - 1; depth2 >= prun; depth2--) {
                ...
                depth2 = depth2 - ret;
                sol = depth1 + depth2;
                if (preIdx != 0) //Alert, Error //If depth2 == 0, the solution is optimal. In this case, we won't try preScramble to find shorter solutions.
                    for (int i = preMoveList[preIdx].Length - 1; i >= 0; i--)
                        appendPreMove(preMoveList[preIdx][i], depth2);

                solution = solutionToString();
            }

            if (depth2 != maxDep2 - 1) { //At least one solution has been found.
                maxDep2 = Math.Max(MAX_DEPTH2, sol - length1);
        ...
        void appendPreMove(int preMove, int depth2) {
            //Alert, Error //If depth2 == 0, the solution is optimal. In this case, we won't try preScramble to find shorter solutions.
        ...
        private int phase2(int edge, int esym, int corn, int csym, int mid, int maxl, int depth, int lm) {
            ...
            for (int m = 0; m < 10; m++) {
                ...
                if (CoordCube.getPruning(CoordCube.MCPermPrun, cornx * 24 + CoordCube.MPermConj[midx, csymx]) >= maxl)
                    continue;
                ...
                if (CoordCube.getPruning(CoordCube.EPermCCombPrun, edgex * 70 + CoordCube.CCombConj[CubieCube.Perm2Comb[cornx], CubieCube.SymMultInv[esymx, csymx]]) >= maxl)
                    continue;

                if (CoordCube.getPruning(CoordCube.MEPermPrun, edgex * 24 + CoordCube.MPermConj[midx, esymx]) >= maxl)
                    continue;

                int edgei = CubieCube.getPermSymInv(edgex, esymx, false);
                int corni = CubieCube.getPermSymInv(cornx, csymx, true);
                if (CoordCube.getPruning(CoordCube.EPermCCombPrun, (edgei >> 4) * 70 + CoordCube.CCombConj[CubieCube.Perm2Comb[corni >> 4], CubieCube.SymMultInv[edgei & 0xf, corni & 0xf]]) >= maxl)
                    continue;
