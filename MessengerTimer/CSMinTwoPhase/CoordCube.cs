...
namespace CSMinTwoPhase {
    internal class CoordCube {
        ...
        //ORIGINAL LINE: internal static char[][] UDSliceMove = new char[N_SLICE][N_MOVES];
        internal static char[][] UDSliceMove = Util.GenerateTwoDimensionArray<char>(N_SLICE, N_MOVES);
        //JAVA TO C# CONVERTER CRACKED BY X-CRACKER NOTE: The following call to the 'RectangularArrays' helper class reproduces the rectangular array initialization that is automatic in Java:
        //ORIGINAL LINE: internal static char[][] TwistMove = new char[N_TWIST_SYM][N_MOVES];
        internal static char[][] TwistMove = Util.GenerateTwoDimensionArray<char>(N_TWIST_SYM, N_MOVES);
        //JAVA TO C# CONVERTER CRACKED BY X-CRACKER NOTE: The following call to the 'RectangularArrays' helper class reproduces the rectangular array initialization that is automatic in Java:
        //ORIGINAL LINE: internal static char[][] FlipMove = new char[N_FLIP_SYM][N_MOVES];
        internal static char[][] FlipMove = Util.GenerateTwoDimensionArray<char>(N_FLIP_SYM, N_MOVES);
        //JAVA TO C# CONVERTER CRACKED BY X-CRACKER NOTE: The following call to the 'RectangularArrays' helper class reproduces the rectangular array initialization that is automatic in Java:
        //ORIGINAL LINE: internal static char[][] UDSliceConj = new char[N_SLICE][8];
        internal static char[][] UDSliceConj = Util.GenerateTwoDimensionArray<char>(N_SLICE, 8);
        ...
        //phase2
        //JAVA TO C# CONVERTER CRACKED BY X-CRACKER NOTE: The following call to the 'RectangularArrays' helper class reproduces the rectangular array initialization that is automatic in Java:
        //ORIGINAL LINE: internal static char[][] CPermMove = new char[N_PERM_SYM][N_MOVES];
        internal static char[][] CPermMove = Util.GenerateTwoDimensionArray<char>(N_PERM_SYM, N_MOVES);
        //JAVA TO C# CONVERTER CRACKED BY X-CRACKER NOTE: The following call to the 'RectangularArrays' helper class reproduces the rectangular array initialization that is automatic in Java:
        //ORIGINAL LINE: internal static char[][] EPermMove = new char[N_PERM_SYM][N_MOVES2];
        internal static char[][] EPermMove = Util.GenerateTwoDimensionArray<char>(N_PERM_SYM, N_MOVES2);
        //JAVA TO C# CONVERTER CRACKED BY X-CRACKER NOTE: The following call to the 'RectangularArrays' helper class reproduces the rectangular array initialization that is automatic in Java:
        //ORIGINAL LINE: internal static char[][] MPermMove = new char[N_MPERM][N_MOVES2];
        internal static char[][] MPermMove = Util.GenerateTwoDimensionArray<char>(N_MPERM, N_MOVES2);
        //JAVA TO C# CONVERTER CRACKED BY X-CRACKER NOTE: The following call to the 'RectangularArrays' helper class reproduces the rectangular array initialization that is automatic in Java:
        //ORIGINAL LINE: internal static char[][] MPermConj = new char[N_MPERM][16];
        internal static char[][] MPermConj = Util.GenerateTwoDimensionArray<char>(N_MPERM, 16);
        ...
        internal static void initUDSliceMoveConj() {
            ...
            for (int i = 0; i < N_SLICE; i++) {
                ...
                for (int j = 0; j < N_MOVES; j += 3) {
                    ...
                    UDSliceMove[i][j] = (char)d.UDSlice;
                }
                for (int j = 0; j < 16; j += 2) {
                    CubieCube.EdgeConjugate(c, CubieCube.SymInv[j], d);
                    UDSliceConj[i][(int)((uint)j >> 1)] = (char)(d.UDSlice & 0x1ff);
                }
            }
            for (int i = 0; i < N_SLICE; i++) {
                for (int j = 0; j < N_MOVES; j += 3) {
                    ...
                    for (int k = 1; k < 3; k++) {
                        ...
                        udslice = Util.permMult[(int)((uint)udslice >> 9)][(int)((uint)cx >> 9)] << 9 | cx & 0x1ff;
        ...
        internal static void initFlipMove() {
            ...
            for (int i = 0; i < N_FLIP_SYM; i++) {
                c.Flip = CubieCube.FlipS2R[i];
                for (int j = 0; j < N_MOVES; j++) {
                    CubieCube.EdgeMult(c, CubieCube.moveCube[j], d);
                    FlipMove[i][j] = (char)d.FlipSym;
                }
            }
        }
        ...
        internal static void initTwistMove() {
            ...
            for (int i = 0; i < N_TWIST_SYM; i++) {
                c.Twist = CubieCube.TwistS2R[i];
                for (int j = 0; j < N_MOVES; j++) {
                    CubieCube.CornMult(c, CubieCube.moveCube[j], d);
                    TwistMove[i][j] = (char)d.TwistSym;
                }
            }
        }
        ...
        internal static void initCPermMove() {
            ...
            for (int i = 0; i < N_PERM_SYM; i++) {
                c.CPerm = CubieCube.EPermS2R[i];
                for (int j = 0; j < N_MOVES; j++) {
                    CubieCube.CornMult(c, CubieCube.moveCube[j], d);
                    CPermMove[i][j] = (char)d.CPermSym;
                }
            }
        }
        ...
        internal static void initMPermMoveConj() {
            ...
            for (int i = 0; i < N_MPERM; i++) {
                ...
                for (int j = 0; j < N_MOVES2; j++) {
                    CubieCube.EdgeMult(c, CubieCube.moveCube[Util.ud2std[j]], d);
                    MPermMove[i][j] = (char)d.MPerm;
                }
                for (int j = 0; j < 16; j++) {
                    CubieCube.EdgeConjugate(c, CubieCube.SymInv[j], d);
                    MPermConj[i][j] = (char)d.MPerm;
                }
            }
        }
        ...
        //ORIGINAL LINE: static void initRawSymPrun(int[] PrunTable, final int INV_DEPTH, final char[][] RawMove, final char[][] RawConj, final char[][] SymMove, final char[] SymState, final byte[] SymSwitch, final int[] moveMap, final int SYM_SHIFT)
        internal static void initRawSymPrun(int[] PrunTable, int INV_DEPTH, char[][] RawMove, char[][] RawConj, char[][] SymMove, char[] SymState, sbyte[] SymSwitch, int[] moveMap, int SYM_SHIFT) {

            //JAVA TO C# CONVERTER CRACKED BY X-CRACKER WARNING: The original Java variable was marked 'final':
            //ORIGINAL LINE: final int SYM_MASK = (1 << SYM_SHIFT) - 1;
            int SYM_MASK = (1 << SYM_SHIFT) - 1;
            //JAVA TO C# CONVERTER CRACKED BY X-CRACKER WARNING: The original Java variable was marked 'final':
            //ORIGINAL LINE: final int N_RAW = RawMove.length;
            int N_RAW = RawMove.Length;
            //JAVA TO C# CONVERTER CRACKED BY X-CRACKER WARNING: The original Java variable was marked 'final':
            //ORIGINAL LINE: final int N_SYM = SymMove.length;
            int N_SYM = SymMove.Length;
            //JAVA TO C# CONVERTER CRACKED BY X-CRACKER WARNING: The original Java variable was marked 'final':
            //ORIGINAL LINE: final int N_SIZE = N_RAW * N_SYM;
            int N_SIZE = N_RAW * N_SYM;
            //JAVA TO C# CONVERTER CRACKED BY X-CRACKER WARNING: The original Java variable was marked 'final':
            //ORIGINAL LINE: final int N_MOVES = RawMove[0].length;
