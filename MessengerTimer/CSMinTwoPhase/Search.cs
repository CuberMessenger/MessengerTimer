...
namespace CSMinTwoPhase {
    public class Search {
        ...
        /// 		Error 9: Invalid firstAxisRestrictionStr or lastAxisRestrictionStr </returns>
        public virtual string solution(string facelets, int maxDepth, long timeOut, long timeMin, int verbose, string firstAxisRestrictionStr, string lastAxisRestrictionStr) {
            lock (this) {
                ...
                this.sol = maxDepth + 1;
                this.timeOut = DateTime.Now.Ticks + timeOut;
                ...
                if (!ReferenceEquals(firstAxisRestrictionStr, null)) {
                    if (!Util.str2move.ContainsKey(firstAxisRestrictionStr)) {
                        return "Error 9";
                    }
                    firstAxisRestriction = Util.str2move[firstAxisRestrictionStr].Value;
                ...
                if (!ReferenceEquals(lastAxisRestrictionStr, null)) {
                    if (!Util.str2move.ContainsKey(lastAxisRestrictionStr)) {
                        return "Error 9";
                    }
                    lastAxisRestriction = Util.str2move[lastAxisRestrictionStr].Value;
        ...
        public virtual string solution(string facelets, int maxDepth, long timeOut, long timeMin, int verbose) {
            lock (this) {
                return solution(facelets, maxDepth, timeOut, timeMin, verbose, null, null);
            }
        }

        internal virtual int verify(string facelets) {
            int count = 0x000000;
            try {
                string center = new string(new char[] { facelets[4], facelets[13], facelets[22], facelets[31], facelets[40], facelets[49] });
                for (int i = 0; i < 54; i++) {
        ...
        private string solve(CubieCube c) {
            ...
            for (int i = 0; i < 6; i++) {
                ...
                for (int j = 0; j < i; j++) { //If S_i^-1 * C * S_i == C, It's unnecessary to compute it again.
                    if (twist[i] == twist[j] && flip[i] == flip[j] && slice[i] == slice[j] && corn0[i] == corn0[j] && ud8e0[i] == ud8e0[j]) {
                        conjMask |= 1 << i;
                        break;
                    }
                }
                if ((conjMask & (1 << i)) == 0) {
                    prun[i] = Math.Max(Math.Max(CoordCube.getPruning(CoordCube.UDSliceTwistPrun, ((int)((uint)twist[i] >> 3)) * 495 + CoordCube.UDSliceConj[slice[i] & 0x1ff][twist[i] & 7]), CoordCube.getPruning(CoordCube.UDSliceFlipPrun, ((int)((uint)flip[i] >> 3)) * 495 + CoordCube.UDSliceConj[slice[i] & 0x1ff][flip[i] & 7])), Tools.USE_TWIST_FLIP_PRUN ? CoordCube.getPruning(CoordCube.TwistFlipPrun, ((int)((uint)twist[i] >> 3)) * 2688 + (flip[i] & 0xfff8 | CubieCube.Sym8MultInv[flip[i] & 7][twist[i] & 7])) : 0);
                }
                c.URFConjugate();
                if (i == 2) {
                    c.invCubieCube();
            ...
            for (depth1 = 0; depth1 < sol; depth1++) {
                ...
                for (urfIdx = 0; urfIdx < 6; urfIdx++) {
                    ...
                    if ((prun[urfIdx] <= depth1) && phase1((int)((uint)twist[urfIdx] >> 3), twist[urfIdx] & 7, (int)((uint)flip[urfIdx] >> 3), flip[urfIdx] & 7, slice[urfIdx] & 0x1ff, depth1, lm) == 0) {
                        return string.ReferenceEquals(solutionString, null) ? "Error 8" : solutionString;
                    }
                }
            }
            return string.ReferenceEquals(solutionString, null) ? "Error 7" : solutionString;
        }
        ...
        private int phase1(int twist, int tsym, int flip, int fsym, int slice, int maxl, int lastAxis) {
            ...
            for (int axis = 0; axis < 18; axis += 3) {
                ...
                for (int power = 0; power < 3; power++) {
                    ...
                    int prun = CoordCube.getPruning(CoordCube.UDSliceTwistPrun, twistx * 495 + CoordCube.UDSliceConj[slicex][tsymx]);
                    if (prun > maxl) {
                        break;
                    }
                    else if (prun == maxl) {
                    ...
                    if (Tools.USE_TWIST_FLIP_PRUN) {
                        prun = CoordCube.getPruning(CoordCube.TwistFlipPrun, (twistx * 336 + flipx) << 3 | CubieCube.Sym8MultInv[fsymx][tsymx]);
                        if (prun > maxl) {
                            break;
                        }
                        else if (prun == maxl) {
                    ...
                    prun = CoordCube.getPruning(CoordCube.UDSliceFlipPrun, flipx * 495 + CoordCube.UDSliceConj[slicex][fsymx]);
                    if (prun > maxl) {
                        break;
                    }
                    else if (prun == maxl) {
        ...
        private int initPhase2() {
            if (DateTime.Now.Ticks >= (solutionString is null ? timeOut : timeMin)) {
                return 0;
            }
            ...
            for (int i = valid1; i < depth1; i++) {
                ...
                mid4[i + 1] = Util.permMult[(int)((uint)mid4[i] >> 9)][(int)((uint)cx >> 9)] << 9 | cx & 0x1ff;
            }
            valid1 = depth1;
            ...
            for (int i = valid2; i < depth1; i++) {
                ...
                u4e = Util.permMult[(int)((uint)u4e >> 9)][(int)((uint)cx >> 9)] << 9 | cx & 0x1ff;

                cx = CoordCube.UDSliceMove[d4e & 0x1ff][m];
                d4e = Util.permMult[(int)((uint)d4e >> 9)][(int)((uint)cx >> 9)] << 9 | cx & 0x1ff;
            ...
            for (int depth2 = prun; depth2 < maxDep2; depth2++) {
                if (phase2(edge, esym, cidx, csym, mid, depth2, depth1, lm)) {
                    sol = depth1 + depth2;
                    maxDep2 = Math.Min(12, sol - depth1);
                    solutionString = solutionToString();
                    return DateTime.Now.Ticks >= timeMin ? 0 : 1;
                }
            }
            return 1;
        }
